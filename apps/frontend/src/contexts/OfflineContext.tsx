...
export const OfflineProvider: React.FC<OfflineProviderProps> = ({ children }) => {
  ...
  const syncQueueRef = useRef<SyncItem[]>(syncQueue);
  ...
  const processSyncItem = useCallback(async (item: SyncItem): Promise<void> => {
    switch (item.type) {
      case 'message':
  ...
  useEffect(() => {
    const handleOnline = () => {
      ...
      // processSyncQueue uses refs internally, safe to call
      processSyncQueue();
    };
