...
import { getApiBaseUrl, readJsonOrText } from '../utils/api';
...
export const AuthProvider: React.FC<AuthProviderProps> = ({ children }) => {
  ...
  const login = async (email: string, password: string): Promise<void> => {
    try {
      ...
      // API Gateway login response does not currently return a profile; construct a minimal user.
      // Note: user_id is not returned on login, but it's embedded in the JWT; for now store email-only user.
    ...
    try {
      ...
      const response = await fetch(`${apiBaseUrl}/auth/register`, {
        ...
        body: JSON.stringify({ 
          ...
          is_anonymous: true // Default to anonymous for privacy
        }),
      });
